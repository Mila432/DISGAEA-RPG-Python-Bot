...
class Database(object):
    ...
    def createDb(self):
        ...
        c = conn.cursor()
        c.execute(
            '''CREATE TABLE "players" ("uuid" TEXT,"password" TEXT,"id" INTEGER UNIQUE,"gems" INTEGER,PRIMARY KEY("id"));''')
        conn.commit()
        conn.close()
    ...
    def addAccount(self, uuid, password, pid, gems):
        conn = sqlite3.connect(self.sqlite_file)
        c = conn.cursor()
        c.execute("INSERT OR IGNORE INTO players (uuid,password,id,gems) VALUES ('%s','%s',%s,%s)" % (
            uuid, password, pid, gems))
        conn.commit()
        conn.close()
